...
class Recommendator:
    ...
    def extract_parameters(self, df, list_films, N=31):
        ...
        film_parameters.sort(key=lambda x: self.selection_criteria(title_main,
                                                                   ...
                                                                   score=x[2],
                                                                   votes_norm=((x[
                                                                                    3] - df.num_voted_users.mean()) / df.num_voted_users.std()),
                                                                   gross_norm=((x[
                                                                                    4] - df.gross.mean()) / df.gross.std())),
                             reverse=True)
    ...
    def fuzzing(series_element, string):
        ...
        """
        try:
            mark = 2 / (1 / fuzz.token_set_ratio(series_element.lower(), string.lower()) + 1 / fuzz.ratio(
                series_element.lower(), string.lower()))
